...
public class eventResponseManager extends HttpServlet {
	...
	// registeredUserManager rm =	registeredUserManager.getInstance();
		...
		else if(action.equals("Update Product"))
		{
			...
			String price=request.getParameter("price").trim();
			String type=request.getParameter("type").trim();
			String exhibitorUname=request.getParameter("exhibitorUname").trim();
			String kind=request.getParameter("kind").trim();
						
			if(productTitle==""||description==""||image==""||price==""||type==""||exhibitorUname==""||kind=="")
			{
				request.setAttribute("errorMessage", "Event can not be updated because you are missing required fields...");
				rd = request.getRequestDispatcher("/errorPage.jsp");
			}
						...
						Product product = null;
						product=new Product(productId,productTitle,description,image,price,type,exhibitorUname,kind);
						Product updatedProduct=em.updateProduct(product);
						//request.setAttribute("indexMessage", "You are successfully registered...");
						Map<String, Product> products=em.getAllProducts();
