...
<legend>
<h1 style="color:white;">Your Products for Event : <c:out value="${requestScope.eventname} "></c:out></h1></legend>

<table border="1" align="center" style="color:white">
<tr>
<th>Image</th><th>Product Name</th><th>Description</th><th>Price</th>
</tr>
...
<c:forEach items="${requestScope.products}" var="product">
	...
	<fieldset style="margin-left:50px;margin-right:50px">
<legend>
<h1 style="color:white;">Your Bidding Products for Event : <c:out value="${requestScope.eventname} "></c:out></h1></legend>

<table border="1" align="center" style="color:white">
<tr>
<th>Image</th><th>Product Name</th><th>Description</th><th>Start Price</th><th>Start Date</th><th>End Date</th>
</tr>

<c:forEach items="${requestScope.bidproducts}" var="product">
	
	<form action="${pageContext.request.contextPath}/exhibitorResponseManager" method="post">
		<tr>
		<%-- <th><c:out value="${event.key}"></c:out></th> --%>
		<td><image width="150" height="150"  src="${pageContext.request.contextPath}/eimages/${product.getProduct().getImage()}"/></td>
		<th><input type="hidden" name="productid" value="<c:out value="${product.getProduct().getProductId()}"></c:out>"/>
		<input type="text" size="8" onfocus="this.blur();" class="noedit abc" name="productname" value="<c:out value="${product.getProduct().getProductTitle()}"></c:out>"/></th>
		<th><input type="text" size="8" onfocus="this.blur();" class="noedit abc"   name="details" value="<c:out value="${product.getProduct().getDescription()}"></c:out>"/></th>
		<th><input type="text" size="8" onfocus="this.blur();" class="noedit abc" name="startprice" value="<c:out value="${product.getStartPrice()}"></c:out>"/></th>
